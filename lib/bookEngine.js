...
const BookEngine = {
    ...
    initPageFlip() {
        ...
        // Book sizing with reasonable constraints
        let pageWidth, pageHeight;

        if (isMobile) {
            // Single page view - constrained for mobile
            pageWidth = Math.min(vw * 0.85, 350);
            pageHeight = Math.min(vh * 0.65, 500);
        } else {
            // Two-page view - each page sized for comfortable reading
            pageWidth = Math.min(vw * 0.25, 450);
            pageHeight = Math.min(vh * 0.70, 650);
        }
