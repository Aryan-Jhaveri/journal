...
const BookEngine = {
    ...
    renderContent(chapters) {
        ...
        // With showCover: true, StPageFlip treats the last page as the back cover
    ...
    createPDFPageElement(pdfPage, chapter, isFirstPage, partNum, totalParts) {
        ...
        pageDiv.classList.add('page', 'pdf-book-page');
    ...
    initPageFlip() {
        ...
        // Calculate page dimensions — derive height from width to match PDF aspect ratio
        const pageWidth = Math.min(vw * sizeConfig.pageWidthPercent, sizeConfig.maxPageWidth);
        const pdfRatio = settings.pdfAspectRatio || (792 / 612); // height/width, default US Letter
        const pageHeight = Math.min(pageWidth * pdfRatio, sizeConfig.maxPageHeight);

        // Force single page on mobile by ensuring minWidth is large enough that 2 pages won't fit
        // If 2 * minWidth > viewportWidth, it forces Portrait mode
        const effectiveMinWidth = isMobile ? (vw / 2 + 10) : (flipConfig.minWidth || 150);
        ...
        this.pageFlipInstance = new St.PageFlip(bookEl, {
            ...
            maxWidth: flipConfig.maxWidth || 600,
            ...
            autoSize: false  // Must be false so leaf/flip animation matches page dimensions
        });
